...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				num = anu.participants[0]
				teks = `NOOOO se nos fue un gil :( @${num.split('@')[0]}👋\n\nQue dios lo bendiga 😎`
				client.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'Tranquilo/a ando procesando todo lo que me pediste.\n _duracion de 1 a 20 segundos_',
				success: '✔️ Listo ✔️',
				...
				only: {
					...
					ownerB: '[❗] Este comando es exclusivo! ❌',
					admin: '[❗] Este comando solo puede ser utilizado por administradores del grupo! ❌',
					Badmin: '[❗] Este comando solo se puede usar cuando el bot se convierte en administrador! ❌',
                                        daftarB: `No estas registrado en mi base de datos  \n\nComando : ${prefix}daftar Nombre\nEjemplo : ${prefix}daftar rosse`,
				}
			}
			...
			const ownerNumber = ["17632806107@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '17632806107@s.whatsapp.net'
                        /******ApiKey Input******/
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
				break*/
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedido recibido : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'Chao puta', text)
					}
					break
				case 'exe':
	              client.updatePresence(from, Presence.composing) 
			        ...
			        if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ error, al momento de convertir ${tipe} a la sticker`)
							})
							.on('end', function () {
								console.log('Finish')
	        ...
	        if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `*Canción encontrada!!!*\nTítulo : ${anu.result.title}\nFuente : ${anu.result.source}\nTamaño : ${anu.result.size}\n\n*ESPERE ENVIANDO ARCHIVO, NO SPAM*`
				...
				case 'nsfwass':
					...
					case 'nsfwarmpits':
				        ...
				        uptime = process.uptime()
                                        ...
                                        break
                                case 'ttp':
					if (args.length < 1) return reply('Debes escribir el texto')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
